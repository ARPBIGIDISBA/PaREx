...
config = init_configs(script_directory, "gene_absence.json", required_keys=["BLAST_OPTIONS", "BLASTN_OPTIONS"])
...
def gene_absence_run(project_name, config=config, only_output = False, direct_file = None, normal_output = False, extra_config= {"force": False, "keep-output": False}):
    ...
    files_nucleotide = [f for f in os.listdir(FASTA_PATH) if f.endswith(".fasta")]
    logger.debug("Nucleotide files to process: %s", files_nucleotide)
